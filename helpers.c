...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for(int i = 0; i < height; i++)
    {
        for(int j = 0; j < width; j++)
        {
            ...
            {
                tempimage[i][j].rgbtBlue = (int)sum[0];
            }
            ...
            {
                tempimage[i][j].rgbtGreen = (int)sum[1];
            }
            ...
            {
                tempimage[i][j].rgbtRed = (int)sum[2];
            }
        }
    }
...
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 1; i <= height; i++)
    {
        for (int j = 1; j <= width; j++)
        {
            ...
            sum[0] = (-tmpimage[i - 1][j - 1].rgbtBlue) + (tmpimage[i - 1][j + 1].rgbtBlue) - (2 * tmpimage[i][j - 1].rgbtBlue) + (2 * tmpimage[i][j + 1].rgbtBlue) - (tmpimage[i + 1][j - 1].rgbtBlue) + (tmpimage[i + 1][j + 1].rgbtBlue);
            sum[1] = (-tmpimage[i - 1][j - 1].rgbtGreen) + (tmpimage[i - 1][j + 1].rgbtGreen) - (2 * tmpimage[i][j - 1].rgbtGreen) + (2 * tmpimage[i][j + 1].rgbtGreen) - (tmpimage[i + 1][j - 1].rgbtGreen) + (tmpimage[i + 1][j + 1].rgbtGreen);
            sum[2] = (-tmpimage[i - 1][j - 1].rgbtRed) + (tmpimage[i - 1][j + 1].rgbtRed) - (2 * tmpimage[i][j - 1].rgbtRed) + (2 * tmpimage[i][j + 1].rgbtRed) - (tmpimage[i + 1][j - 1].rgbtRed) + (tmpimage[i + 1][j + 1].rgbtRed);

            sum[3] = (-tmpimage[i - 1][j - 1].rgbtBlue) - (2 * tmpimage[i - 1][j].rgbtBlue) - (tmpimage[i - 1][j + 1].rgbtBlue) + (tmpimage[i + 1][j - 1].rgbtBlue) + (2 * tmpimage[i + 1][j].rgbtBlue) + (tmpimage[i + 1][j + 1].rgbtBlue);
            sum[4] = (-tmpimage[i - 1][j - 1].rgbtGreen) - (2 * tmpimage[i - 1][j].rgbtGreen) - (tmpimage[i - 1][j + 1].rgbtGreen) + (tmpimage[i + 1][j - 1].rgbtGreen) + (2 * tmpimage[i + 1][j].rgbtGreen) + (tmpimage[i + 1][j + 1].rgbtGreen);
            sum[5] = (-tmpimage[i - 1][j - 1].rgbtRed) - (2 * tmpimage[i - 1][j].rgbtRed) - (tmpimage[i - 1][j + 1].rgbtRed) + (tmpimage[i + 1][j - 1].rgbtRed) + (2 * tmpimage[i + 1][j].rgbtRed) + (tmpimage[i + 1][j + 1].rgbtRed);
